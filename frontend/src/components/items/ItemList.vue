  ...
  <div class="flex flex-col h-full min-h-0 overflow-hidden">
    ...
    <div class="flex items-center gap-4 mb-3 px-1 flex-wrap">
      <div class="flex items-center gap-1.5">
        ...
        <span class="text-sm font-semibold text-primary">৳{{ totalAmount.toFixed(2) }}</span>
      </div>
      <div class="flex items-center gap-1.5">
        <span class="text-xs text-muted">Deposit:</span>
        <span class="text-sm font-semibold text-info-text">৳{{ totalDeposit.toFixed(2) }}</span>
      </div>
      <div class="flex items-center gap-1.5">
        <span class="text-xs text-muted">Paid:</span>
        <span class="text-sm font-semibold text-success-text">৳{{ totalCustomerPaid.toFixed(2) }}</span>
      </div>
      <div class="flex items-center gap-1.5">
        <span class="text-xs text-muted">Due:</span>
        <span class="text-sm font-semibold" :class="totalDue > 0 ? 'text-warning-text' : 'text-success-text'">৳{{
    ...
    <div class="flex-1 min-h-0 overflow-auto">
      <div class="min-w-3xl">
        <div
          ...
          <div class="col-span-3">Item / Customer</div>
          <div class="col-span-2">Category</div>
          <div class="col-span-2">Storage / Contract</div>
      ...
      <div v-if="selectedItem" class="space-y-5">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1">
            ...
            <p class="text-sm text-primary">{{ selectedItem.category || '—' }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase">Subcategory</p>
            <p class="text-sm text-primary">{{ selectedItem.subcategory || '—' }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-3">
          <div class="space-y-1">
            ...
            <p class="text-sm text-primary">{{ selectedItem.quantity }} {{ selectedItem.quantity_unit }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase">Weight</p>
            <p class="text-sm text-primary">{{ selectedItem.weight ? selectedItem.weight + ' ' +
              (selectedItem.weight_unit || 'kg') : '—' }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase">Dimensions</p>
        ...
        <div class="p-4 rounded-xl border border-default bg-surface-alt">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="space-y-1">
              <p class="text-[10px] text-muted uppercase">Amount</p>
              <p class="text-sm font-semibold text-primary">৳{{ selectedItem.amount.toFixed(2) }}</p>
            </div>
            <div class="space-y-1">
              <p class="text-[10px] text-muted uppercase">Deposit</p>
              <p class="text-sm font-semibold text-info-text">৳{{ selectedItem.deposit.toFixed(2) }}</p>
            </div>
            <div class="space-y-1">
              <p class="text-[10px] text-muted uppercase">Paid</p>
              <p class="text-sm font-semibold text-success-text">৳{{ (selectedItem.customer_paid || 0).toFixed(2) }}</p>
            </div>
            <div class="space-y-1">
              <p class="text-[10px] text-muted uppercase">Due</p>
              <p class="text-sm font-semibold"
                :class="getItemDue(selectedItem) > 0 ? 'text-warning-text' : 'text-success-text'">৳{{
                  getItemDue(selectedItem).toFixed(2) }}</p>
            </div>
          </div>
        </div>

        <!-- Storage Contract -->
        <div v-if="selectedItem.duration_type" class="p-4 rounded-xl border border-default bg-surface-alt">
          <h3 class="text-xs font-semibold text-muted uppercase tracking-wider mb-3">Storage Contract</h3>
          <div class="grid grid-cols-2 gap-3">
            <div class="space-y-1">
            ...
            <div class="space-y-1">
              <p class="text-[10px] text-muted uppercase">Status</p><span
                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border"
                :class="selectedItem.status === 'active' ? 'border-green-400 bg-green-50 text-green-700' : 'border-blue-400 bg-blue-50 text-blue-700'">{{
                  selectedItem.status }}</span>
            </div>
            <div class="space-y-1">
              <p class="text-[10px] text-muted uppercase">Start Date</p>
              <p class="text-xs text-primary">{{ selectedItem.start_date ? formatDate(selectedItem.start_date) : '—' }}
              </p>
            </div>
          </div>
        </div>
...
const totalDue = computed(() => {
  return itemsStore.items.reduce((sum, i) => sum + getItemDue(i), 0)
})
...
const copyToClipboard = async () => {
  ...
  const headers = ['ID', 'Customer Name', 'Phone', 'Category', 'Subcategory', 'Qty', 'Unit', 'Duration', 'Status', 'Amount', 'Deposit', 'Paid', 'Due', 'Notes']
  const rows = items.map(i => [i.id, i.name, i.customer_phone, i.category || '', i.subcategory || '', i.quantity, i.quantity_unit, i.duration_type ? `${i.duration} ${i.duration_type}` : '', i.status || '', i.amount || '', i.deposit || '', i.customer_paid || 0, getItemDue(i).toFixed(2), i.notes || ''])
...
const filteredItems = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    result = result.filter(i => (i.name && i.name.toLowerCase().includes(q)) || (i.customer_phone && i.customer_phone.includes(q)) || (i.category && i.category.toLowerCase().includes(q)))
  }
  if (currentSort.value === 'newest') result.sort((a, b) => b.id - a.id)
  else if (currentSort.value === 'oldest') result.sort((a, b) => a.id - b.id)
  else if (currentSort.value === 'name_asc') result.sort((a, b) => a.name.localeCompare(b.name))
  else if (currentSort.value === 'name_desc') result.sort((a, b) => b.name.localeCompare(a.name))
  return result
})
...
const getDimensions = (item: Item): string => {
  ...
  return parts.length > 0 ? parts.join(' × ') + ' ' + (item.dimension_unit || 'in') : '—'
}

const formatDate = (d: string): string => d ? new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'
</script>

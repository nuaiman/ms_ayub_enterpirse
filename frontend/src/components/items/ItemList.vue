  ...
  <div class="flex flex-col h-full min-h-0 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 mt-1 shrink-0">
      ...
      <div class="flex items-center gap-2 flex-wrap">
        <div class="relative flex-1 sm:flex-none w-full sm:w-auto">
          ...
          <input v-model="searchQuery" type="text" placeholder="Search items..." class="input w-full sm:w-80 pl-9 pr-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-accent focus:border-transparent placeholder:text-muted" />
    ...
    <div class="flex-1 min-h-0 overflow-auto">
      <div class="min-w-3xl">
        ...
        <div v-else-if="filteredItems.length === 0" class="flex items-center justify-center py-12"><p class="text-sm text-muted">No items found</p></div>
        <div v-else><ItemRow v-for="item in filteredItems" :key="item.id" :item="item" @item-updated="handleItemUpdated" @view-item="openDetailDialog" /></div>
      </div>
    </div>

    <div v-if="!loading && filteredItems.length > 0" class="flex items-center justify-between py-3 px-1 border-t border-divider shrink-0"><p class="text-xs text-muted">Showing {{ filteredItems.length }} of {{ itemsStore.items.length }}</p></div>

    <BaseDialog v-model="showCreateDialog" size="lg"><CreateItemForm @item-created="handleItemCreated" /></BaseDialog>

    <!-- Detail Dialog -->
    <BaseDialog v-model="showDetailDialog" size="lg">
      <div v-if="selectedItem" class="space-y-5">
        <div class="flex items-center gap-4">
          <div v-if="selectedItem.image_url" class="shrink-0 w-20 h-20 rounded-lg overflow-hidden border border-default"><img :src="getImageUrl(selectedItem.image_url) || undefined" :alt="selectedItem.product_name" class="w-full h-full object-cover" /></div>
          <div>
            <h2 class="text-xl font-bold text-primary">{{ selectedItem.product_name }}</h2>
            <p v-if="selectedItem.customer_phone" class="text-sm text-muted">{{ selectedItem.customer_phone }}</p>
            <p v-if="selectedItem.customer_email" class="text-xs text-muted">{{ selectedItem.customer_email }}</p>
            <div v-if="selectedItem.lot_number" class="mt-1"><span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-surface-alt text-secondary">Lot: {{ selectedItem.lot_number }}</span></div>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1"><p class="text-xs text-muted uppercase">Storage Name</p><p class="text-sm text-primary">{{ selectedItem.storage_name || '—' }}</p></div>
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          <div class="space-y-1"><p class="text-xs text-muted uppercase">Weight</p><p class="text-sm text-primary">{{ selectedItem.weight ? selectedItem.weight + ' ' + (selectedItem.weight_unit || 'kg') : '—' }}</p></div>
        </div>
...
const sortOptions = [{ key: 'newest', label: 'Newest First' }, { key: 'oldest', label: 'Oldest First' }, { key: 'name_asc', label: 'Name A-Z' }, { key: 'name_desc', label: 'Name Z-A' }]
...
const copyToClipboard = async () => {
  const items = filteredItems.value; if (items.length === 0) return
  const headers = ['ID', 'Product Name', 'Phone', 'Lot', 'Storage', 'Account', 'Category', 'Subcategory', 'Qty', 'Unit', 'Weight', 'Amount', 'Deposit', 'Paid', 'Due', 'Notes']
  const rows = items.map(i => [i.id, i.product_name, i.customer_phone || '', i.lot_number || '', i.storage_name || '', i.account_name || '', i.category || '', i.subcategory || '', i.quantity, i.quantity_unit, i.weight ? i.weight + ' ' + (i.weight_unit || 'kg') : '', i.amount || '', i.deposit || '', i.customer_paid || 0, getItemDue(i).toFixed(2), i.notes || ''])

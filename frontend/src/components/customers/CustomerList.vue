  ...
  <div class="flex flex-col h-full min-h-0 overflow-hidden">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 mt-1 shrink-0">
      <div class="flex items-center gap-3">
        ...
        <span class="text-sm text-muted bg-surface-alt px-2 py-0.5 rounded-md">{{ customersStore.customers.length }}
          total</span>
      </div>
    ...
    <div v-if="!loading && filteredCustomers.length > 0"
      ...
      <p class="text-xs text-muted">Showing {{ filteredCustomers.length }} of {{ customersStore.customers.length }}
        customers</p>
    ...
    <BaseDialog v-model="showDetailDialog" size="lg">
      <div v-if="selectedCustomer" class="space-y-5">
        ...
        <div class="flex items-center gap-4 pb-4 border-b border-divider">
          ...
          <span v-if="selectedCustomer.user_id" class="text-xs text-muted">Managed by: #{{ selectedCustomer.user_id
          }}</span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-1">
            ...
            <p class="text-sm text-primary">{{ selectedCustomer.email || '—' }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase tracking-wider">Phone</p>
            <p class="text-sm text-primary">{{ selectedCustomer.phone }}</p>
          </div>
          <div class="space-y-1 md:col-span-2">
            <p class="text-xs text-muted uppercase tracking-wider">Address</p>
            <p class="text-sm text-primary">{{ selectedCustomer.address || '—' }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase tracking-wider">ID Type</p>
            <p class="text-sm text-primary capitalize">{{ selectedCustomer.id_type || '—' }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase tracking-wider">ID Number</p>
            <p class="text-sm text-primary">{{ selectedCustomer.id_number || '—' }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase tracking-wider">Created</p>
            <p class="text-sm text-primary">{{ formatDate(selectedCustomer.created_at) }}</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-muted uppercase tracking-wider">Updated</p>
            <p class="text-sm text-primary">{{ formatDate(selectedCustomer.updated_at) }}</p>
          </div>
          <div class="space-y-1 md:col-span-2">
            <p class="text-xs text-muted uppercase tracking-wider">Notes</p>
            <p class="text-sm text-primary whitespace-pre-wrap">{{ selectedCustomer.notes || '—' }}</p>
          </div>
        </div>
